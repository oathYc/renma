...
//检查来源是否为单词
function uc_user_source($uid) {
	$return = call_user_func(UC_API_FUNC, 'user', 'get_source_words', array('uid'=>$uid));
	return uc_unserialize($return);
}
...
function uc_user_edit_by_uid($uid, $oldpw, $newpw, $email, $phone) {
	return call_user_func(UC_API_FUNC, 'user', 'edit_by_uid', array('uid'=>$uid, 'oldpw'=>$oldpw, 'newpw'=>$newpw, 'email'=>$email, 'phone'=>$phone));
}
