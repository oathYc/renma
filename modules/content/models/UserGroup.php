        ...
        if($userGroupId){
            ...
            if($userGroup){
                if($userGroup->status ==1){//组团购买成功
                    ...
                    $sql = " select ug.*,m.avatar,m.nickname,o.payPrice from {{%user_group}} ug inner join {{%order}} o on o.id = ug.orderId left join {{%member}} m on m.id = o.uid where o.status = 1 and ug.status = 1 and ug.userGroupId = $userGroupId and ug.id != $userGroupId and o.typeStatus >= 1";//订单已完成  用户确认收货  同一组团
                    $data = \Yii::$app->db->createCommand($sql)->queryAll();
