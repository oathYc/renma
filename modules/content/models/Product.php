...
//        $data = \Yii::$app->db->createCommand($sql)->queryAll();
    ...
    public static function getCommentByType($productId,$type=0,$page=1,$pageSize=10){
        if(!$page){$page=1;}
        $limit = " limit ".($pageSize*($page-1)).",$pageSize";
        $where = " o.productId = $productId and evalTime > 0 ";
        $order = "  order by evalTime desc ";
        if($type ==2){
        ...
        $sql = "select o.id as orderId,o.uid,m.nickname,m.avatar,o.productId,o.evaluate,FROM_UNIXTIME(o.evalTime, '%Y-%m-%d %H:%i:%s') as evalTime from {{%order}} o inner join {{%product}} p on p.id = o.productId left join {{%member}} m on m.id = o.uid where $where $order ";
        $data = \Yii::$app->db->createCommand($sql)->queryAll();
        $total = count($data);
        $sql .= $limit;
        $data = \Yii::$app->db->createCommand($sql)->queryAll();
