...
class uc_note {
    ...
    function synlogin($get, $post) {
        ...
        if(!$u){
            ...
            if (!$u) {
                $time = time();
                $sql = "INSERT INTO x2_user (`userName`,`nickname`,`email`,`userPass`,`phone`,`createTime`,`uid`) VALUES ('{$username}','{$nickname}','{$email}','{$password}','{$phone}','{$time}','{$uid}')";
                $this->dbLink->query($sql);
                ...
                if($nickname != $u['nickname']){
                    $sql = "UPDATE x2_user SET nickname = '$nickname' WHERE id = {$u['id']}";
                    $this->dbLink->query($sql);
                }
                if($uid != $u['uid']){
